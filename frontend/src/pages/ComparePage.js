...
import { 
  GitCompare, Plus, Trash2, Loader2, CheckCircle, AlertTriangle, 
  XCircle, Trophy, ShoppingBag, ExternalLink, ArrowRight, Sparkles
} from 'lucide-react';
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-slate-400 max-w-2xl mx-auto">
            Enter 2-3 Amazon product URLs to compare their reviews, complaints, and verdicts.
          </p>
        </div>
              ...
              <div className="bg-gradient-to-r from-amber-500/20 to-yellow-500/20 border border-amber-500/30 rounded-2xl p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-medium text-amber-400 mb-1">RECOMMENDED CHOICE</h3>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border ${verdictConfig.bg} ${verdictConfig.border}`}>
                        ...
                        <span className={verdictConfig.color}>{product.verdict}</span>
                      </div>
                    ...
                    {product.authenticity_score && (
                      <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg mb-4">
                        <span className="text-slate-400 text-sm">Review Authenticity</span>
                        <span className={`font-semibold ${product.authenticity_score >= 70 ? 'text-emerald-400' : product.authenticity_score >= 40 ? 'text-amber-400' : 'text-red-400'}`}>
                          {product.authenticity_score}%
                        </span>
                      </div>
                    )}
                    ...
                    {/* Top Complaint */}
                    <div className="mb-4">
                      <p className="text-slate-500 text-xs mb-1">TOP COMPLAINT</p>
                      <p className="text-slate-300 text-sm">{product.top_complaints?.[0]?.title || 'N/A'}</p>
                    </div>
          ...
          <div className="max-w-4xl mx-auto mt-16">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                ...
                { step: '2', title: 'AI Analysis', desc: 'Our AI analyzes reviews for each product simultaneously' },
                { step: '3', title: 'See Winner', desc: 'Get a clear recommendation based on review data' }
              ].map((item) => (
